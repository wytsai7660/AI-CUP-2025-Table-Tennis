...
if __name__ == "__main__":
    configs = Configs()
    configs.pos_embed = False  # Enable positional embedding
    configs.use_wandb = False  # Enable Weights & Biases logging
    ...
    # Get one label per unique player for stratification
    # player_levels = train_df.drop_duplicates("player_id")['play years'].values
    ...
    encoders = joblib.load("weights/encoders.pkl")
    
    train_dataset = TrajectoryDataset(
        TRAIN_DATA_DIR,
        train_df[train_df["player_id"].isin(train_player_ids)],
    ...
    # joblib.dump(train_dataset.scaler, "weights/scaler.pkl")
    # joblib.dump(train_dataset.encoders, "weights/encoders.pkl")
    # exit(0)

    
    valid_dataset = TrajectoryDataset(
        TRAIN_DATA_DIR,
        train_df[train_df["player_id"].isin(val_player_ids)],
        max_seq_len=configs.seq_len,
        label=True,
        predicting_fields=PREDICTING_FIELDS,
    ...
    best_task_loss = np.full(len(criterions), 0.0, dtype=np.float32)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (s, e, _, _) in enumerate(criterions):
            ...
            if e - s > 1:
                try:
                    ...
                    roc_auc = roc_auc_score(y_true_oh, y_score, average='micro', multi_class='ovr', labels=np.arange(e - s))
        ...
        for i, (task_loss, f1, roc_auc) in enumerate(zip(task_losses, task_f1, task_roc_auc)):
            if roc_auc > best_task_loss[i]:
                best_task_loss[i] = roc_auc
                best_title[i] = f"epoch_{epoch+1}_loss_{task_loss:.4f}_f1_{f1:.4f}_roc_auc_{roc_auc:.4f}"
                best_model[i] = deepcopy(model.state_dict())
                print(f"New best model found for task {i} at epoch {epoch+1} with roc: {roc_auc:.4f}")
        ...
        if configs.use_wandb:
            wandb.log({
                ...
                "task_roc_auc": {f"task_{PREDICTING_FIELDS[i]}": roc_auc for i, roc_auc in enumerate(task_roc_auc)},
            })
